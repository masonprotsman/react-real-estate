  ...
  return (
    <div className="absolute top-0 left-0 w-full z-10">
        <div className="container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent">
            ...
            <img src={assets.menu_icon} onClick={() => setShowMobileMenu(true)} alt="Menu" className="md:hidden w-7 cursor-pointer"/>
        </div>
      {/* mobile menu */}
      <div className={`${showMobileMenu ? 'fixed w-full' : 'h-0 w-0'}md:hidden fixed w-full right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        <div className='flex justify-end p-6 cursor-pointer'>
            <img src={assets.cross_icon} onClick={() => setShowMobileMenu(false)} alt="Close" className='w-6'/>
        </div>
